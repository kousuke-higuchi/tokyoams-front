<template>
  <v-card>
    <div class="d-flex">
      ...
      <v-sheet class="ma-1">
        <v-select v-model="page" :items="getPageNumber(items)" item-title="page" variant="outlined" density="compact"
          hide-details>
        </v-select>
      </v-sheet>
    </div>
    <v-window v-model="page">
          ...
          <v-table density="compact" class="ledger">
            <thead>
              <tr>
                ...
                <th colspan="3" class="text-center">変状程度</th>
                <th rowspan="2" class="text-center">変状パターン</th>
                <th rowspan="2" class="text-center">変状の種類</th>
                <th rowspan="2" class="text-center">分類</th>
              </tr>
              <tr>
                <th class="text-center">名称</th>
                <th class="text-center">記号</th>
                <th class="text-center">要素番号</th>
                <th class="text-center">変状程度の評価</th>
                <th class="text-center">定量的に取得した値</th>
      ...
      <v-window-item v-for="item in items" :key="item.blockId" :value="item.blockId">
        <v-container>
          <v-table density="compact" class="ledger">
            <thead>
              <tr>
...
export default defineComponent({
  ...
  methods: {
    //TODO:現在使わないためコメントアウト。最終的に使用しない場合は削除
    //ドロップダウンリストの項目を取得する
