<template>
  <div>
    <v-row>
      ...
      <!-- 補修履歴 -->
      <v-col md="9" cols="12">
        <attach-repair-list :attachReapir="repairs" :kindList="streetlights" />
      </v-col>
    </v-row>
  </div>
</template>
...
<script lang="ts" setup>
  ...
  import repairHistories from "~~/assets/streetlightRepair.json"; 
  ...
  const streetlights = computed(() => {
      //配電盤でfilterをする
      return kindListBase.value.filter((s) => s.streetlightuse === 1);
  });
