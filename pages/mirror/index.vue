<template>
  <v-card>
    ...
    <mirror-search-condition-dialog v-model:isOpen="showFind" />
  </v-card>
</template>

<script lang="ts" setup>
	import mirrorervice from "~~/services/mirror-service";
	import { AtfSummary } from "~~/types";
	
	const showMarkerList = ref(false);
	const tab = ref("map");
	const zoom = ref(15);
			// TODO:地図の開始位置小川駅とした暫定対応（データがないため西多摩建設事務所の中心×)
	const center = ref([35.73765, 139.46359]);
	const mirrors = ref<[AtfSummary]>();
	mirrors.value = await mirrorervice.getList();
	...
	const onClickMarker = (m) => {
	  ...
	  navigateTo(`/mirror/${m.id}/ledger`, m);
	};
