<template>
  <v-card>
            ...
            >
              <template #table-row="props">
                ...
                <span v-else-if="props.column.field == 'formattedId'">
                  <nuxt-link :to="`/roadsign/${props.row.id}/ledger`">
    ...
    <roadsign-search-condition-dialog v-model:isOpen="showFind" />

  </v-card>
</template>

<script lang="ts" setup>
  import roadsignervice from "~~/services/roadsign-service";
  ...
  const roadsigns = ref<[AtfSummary]>();
  roadsigns.value = await roadsignervice.getList();
  ...
  const onClickMarker = (m) => {
    ...
    navigateTo(`/roadsign/${m.id}/ledger`, m);
  };
