  ...
  <div>
    <v-row>
      ...
      <v-col :md="child_extend ? 9 : 12">
        <attach-repair-list :attachReapir="repairs" :kindList="switchboards" />
      </v-col>
    </v-row>
  </div>
</template>
...
import repairHistories from "~~/assets/switchboardRepair.json";
...
const switchboards = computed(() => {
  //配電盤でfilterをする
  return kindListBase.value.filter((s) => s.switchboarduse === 1);
});
