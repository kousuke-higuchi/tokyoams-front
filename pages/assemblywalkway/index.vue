    ...
    <v-card-text>
      <v-window v-model="tab">
        ...
        <v-window-item value="list">
          ...
          <div>
            <vue-good-table :columns="columns" :rows="assemblywalkways" :pagination-options="{
              enabled: true
            }">
              <template #table-row="props">
                <span v-if="props.column.field == 'facillityname'">
                  <nuxt-link :to="`/assemblywalkway/${props.row.id}/ledger1`">{{  props.formattedRow[props.column.field] 
                    }}</nuxt-link>
                </span>
        ...
        <v-window-item value="map">
          ...
          <v-card style="height:78vh; width: 94vw">
            <ams-map :zoom="zoom" :center="center" :markers="assemblywalkways" marker-title="facillityname"
...
<script lang="ts">
import assemblywalkwayJson from "../../assets/roadfacility.json";
...
export default defineComponent({
  data() {
    return {
      ...
      assemblywalkways: assemblywalkwayJson,
      zoom: 15,
  ...
  mounted: function () {
    ...
    this.rowsOverlay = assemblywalkwayJson;
  },
  methods: {
    clickAdvanceSerch() {
    ...
    clickMarker(m) {
      ...
      navigateTo(`/assemblywalkway/${m.id}/ledger1`, m)
    }
  }
})
</script>