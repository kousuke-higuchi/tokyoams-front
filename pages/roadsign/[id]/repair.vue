<template>
  <div>
    <v-row>
      ...
      <!-- 補修履歴 -->
      <v-col md="9" cols="12">
        <attach-repair-list :attachReapir="repairs" :kindList="roadsigns" />
      </v-col>
    </v-row>
  </div>
</template>
...
import repairHistories from "~~/assets/roadsignRepair.json"; 
...
const repairs = ref((repairHistories as any) as [AttachRepair]);
  ...
  const roadsigns = computed(() => {
      //道路標識でfilterをする
      return kindListBase.value.filter((s) => s.signuse === 1);
  });
