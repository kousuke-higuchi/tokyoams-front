...
import roadsignsRepairJson from "~~/assets/roadsignRepair.json"; 

export default defineComponent({
  data() {
    return {
      subtitles: [
        { title: "道路反射鏡台帳", props: { to: "ledger" } },
        { title: "補修履歴", props: { to: "repair" } },
      ],
      rowsData:JSON.parse(JSON.stringify(roadsignsRepairJson)) as Array<AttachRepair>,
//TODO:施工区分をatf_cd_constkindから取得
  ...
  computed:{
      afterFilter() {
        //道路標識でfilterをする
        return this.kindListBase.filter((s) => s.signuse === 1);

      },
  },
  mounted: function () {
    console.log("mounted /roadsign/ledger");
  },
  methods: {},
});
</script>
