...
import blinkerRepair from "~~/assets/blinkerRepair.json"; 

export default defineComponent({
  data() {
    return {
      subtitles: [
        { title: "障害物表示灯台帳", props: { to: "ledger" } },
...
//TODO:施工区分をatf_cd_constkindから取得
  ...
  computed:{
      afterFilter() {
        //障害物表示灯でfilterをする
        return this.kindListBase.filter((s) => s.blinkeruse === 1);

      },
  },
  mounted: function () {
    console.log("mounted /blinker/ledger");
  },
  methods: {},
});
</script>
