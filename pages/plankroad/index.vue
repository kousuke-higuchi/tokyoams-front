    ...
    <v-card-text>
      <v-window v-model="tab">
        ...
        <v-window-item value="list">
          ...
          <div>
            <vue-good-table :columns="columns" :rows="plankroads" :pagination-options="{
              enabled: true
            }">
              <template #table-row="props">
                <span v-if="props.column.field == 'facillityname'">
                  <nuxt-link :to="`/plankroad/${props.row.id}/ledger1`">{{  props.formattedRow[props.column.field]  }}
                  </nuxt-link>
                </span>
        ...
        <v-window-item value="map">
          ...
          <v-card style="height:78vh; width: 94vw">
            <ams-map :zoom="zoom" :center="center" :markers="plankroads" marker-title="facillityname"
...
<script lang="ts">
import plankroadJson from "../../assets/roadfacility.json";
...
export default defineComponent({
  ...
  mounted: function () {
    ...
    this.rowsOverlay = plankroadJson;
  },
  methods: {
    clickAdvanceSerch() {
    ...
    clickMarker(m) {
      ...
      navigateTo(`/plankroad/${m.id}/ledger1`, m)
    }
  }
})
</script>