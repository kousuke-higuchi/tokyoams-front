...
import signsRepairJson from "~~/assets/switchboardRepair.json"; 

export default defineComponent({
  data() {
    return {
      subtitles: [
        { title: "配電盤台帳", props: { to: "ledger" } },
        { title: "補修履歴", props: { to: "repair" } },
      ],
      rowsData:JSON.parse(JSON.stringify(signsRepairJson)) as Array<AttachRepair>,
//TODO:施工区分をatf_cd_constkindから取得
  ...
  computed:{
      afterFilter() {
        //配電盤でfilterをする
        return this.kindListBase.filter((s) => s.switchboarduse === 1);

      },
  },
  mounted: function () {
    console.log("mounted /switchboard/ledger");
  },
  methods: {},
});
</script>
