...
import mirrorsRepairJson from "~~/assets/mirrorRepair.json"; 
...
export default defineComponent({
  data() {
    return {
      ...
      rowsData:JSON.parse(JSON.stringify(mirrorsRepairJson)) as Array<AttachRepair>,
//TODO:施工区分をatf_cd_constkindから取得
  ...
  computed:{
      afterFilter() {
        //道路反射鏡でfilterをする
        return this.kindListBase.filter((s) => s.mirroruse === 1);

      },
  },
  mounted: function () {
    console.log("mounted /mirror/ledger");
  },
  methods: {},
});
</script>
