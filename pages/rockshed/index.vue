    ...
    <v-card-text>
      <v-window v-model="tab">
        ...
        <v-window-item value="list">
          <v-container>
            <v-row justify="end">
              <v-btn v-on:click="onAdvanceSerchBtnClick()" color="primary">
                詳細検索
              </v-btn>
              <v-btn class="ml-2" v-on:click="onOutputCSVBtnClick()" color="primary">
                一覧出力
              </v-btn>
            </v-row>
          </v-container>
          <div>
            <vue-good-table :columns="columns" :rows="rocksheds" :pagination-options="{
              enabled: true
            }">
              <template #table-row="props">
                ...
                <span v-else-if="props.column.field == 'outRecodeBtn'">
                  <v-btn class="btn" color="primary" dark size="small" v-on:click="onOutRecodeBtnClick()">
                    Excel
                  </v-btn>
                </span>
              </template>
        ...
        <v-window-item value="map">
          <v-container>
            <v-row justify="end">
              <v-btn v-on:click="onFindBtnClick()" color="primary">
                検索条件
              </v-btn>
              <v-menu v-model="showMarkerList" :close-on-content-click="false" location="bottom end">
                ...
                <v-card>
                  <v-container>
                    <vue-good-table :columns="columnsOverlay" :rows="rowsOverlay" @row-click="onMarkerListRowClick">
                    </vue-good-table>
                  </v-container>
                </v-card>
              </v-menu>
          ...
          <v-card style="height:78vh; width: 94vw">
            <ams-map :zoom="zoom" :center="center" :markers="rocksheds" marker-title="facillityname"
...
<script lang="ts">
import rockshedJson from "../../assets/rockshed.json";
...
export default defineComponent({
  ...
  mounted: function () {
    ...
    this.rowsOverlay = rockshedJson;
  },
  methods: {
    onAdvanceSerchBtnClick() {
